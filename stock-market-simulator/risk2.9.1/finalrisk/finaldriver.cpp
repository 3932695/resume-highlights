...
int procAssign(int a, int nprocs, int rank){
  int savedLow = rank * (a/nprocs);
  int remainder = a % nprocs;
  if (rank < remainder) {
    savedLow += rank;
  }
  else{
...
int main(int argc, char **argv) {
  ...
  int assignedProcLow = procAssign(numa[0], nprocs, rank);
  int assignedProcHigh = procAssign(numa[0], nprocs,rank+1);
  
  
  
  /*
    
    ofstream outfile( rank + "fexplosive.txt", ios_base::app );
    ofstream outfile1( rank + "fnumExplosions.txt", ios_base::app ); //for counting explosions in 100 runs
    ofstream outfile2( rank + "fvolatility.txt" , ios_base::app ); //for recording volatility of each run //7/16/13
  */
  
  //Dec 2014 Kevin and Phil: organizes processor output into individual text files
  
  stringstream sstm;
  string tempName = "fexplosive.txt";
  sstm <<  rank << tempName;
  tempName = sstm.str();
  const char * tempCharExpl = tempName.c_str();
  ofstream outfile(tempCharExpl, ios_base::app );
  
  sstm.str("");
  sstm.clear();
  
  tempName = "fnumExplosions.txt";
  sstm <<  rank << tempName;
  tempName = sstm.str();
  const char * tempCharNumExpl = tempName.c_str();
  tempCharNumExpl = tempName.c_str();
  ofstream outfile1(tempCharNumExpl, ios_base::app );
  
  sstm.str("");
  sstm.clear();
  
  tempName = "fvolatility.txt";
  sstm << rank << tempName;
  tempName = sstm.str();
  const char * tempCharVolatility = tempName.c_str();
  tempCharVolatility = tempName.c_str();
  ofstream outfile2(tempCharVolatility, ios_base::app );
  ...
  for (int m = 0; m<assignedProcLow; m++) {
    if(numa[0] > 1) {
      a[0] += (a[2] - a[1]) / (numa[0]-1);
    }
  }
  //numacount--;
    //}
  ...
  for (int acount = 0; acount < numa[0]; acount++) {	          //5/20/2013 Jim
    ...
    if ((acount >= assignedProcLow) && (acount < assignedProcHigh)) {
